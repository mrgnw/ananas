-- Fix users table to support optional password_hash and add passkey tables
-- This migration ensures the users table allows NULL password_hash for passkey-only users
...
-- Step 2: Copy existing data (only if users table exists and has data)
INSERT OR IGNORE INTO users_new (id, email, password_hash, username, created_at, updated_at)
SELECT id, email, password_hash, username, created_at, updated_at FROM users WHERE EXISTS (SELECT 1 FROM users LIMIT 1);
...
-- Step 5: Create passkeys table for WebAuthn credentials
...
-- Step 6: Create indexes for passkeys
CREATE INDEX IF NOT EXISTS idx_passkeys_user_id ON passkeys(user_id);
CREATE INDEX IF NOT EXISTS idx_passkeys_credential_id ON passkeys(id);

-- Step 7: Create authentication challenges table for WebAuthn flows
...
-- Step 8: Create indexes for auth challenges
