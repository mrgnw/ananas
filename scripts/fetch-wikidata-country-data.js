...
const QUERY = `
SELECT DISTINCT ?country ?countryLabel ?isoCode ?flagImage
...
GROUP BY ?country ?countryLabel ?isoCode ?flagImage
...
async function fetchWikidataCountries() {
  ...
  const countries = data.results.bindings.map(country => {
    ...
    // Create language objects with labels and speaker counts
